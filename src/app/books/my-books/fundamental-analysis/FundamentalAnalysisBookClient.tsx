...
import { chapters as chaptersMetadata } from './chaptersData';
import { ContentFormatter } from './ContentFormatter';

export function FundamentalAnalysisBookClient() {
  ...
  useEffect(() => {
    const loadChapter = async () => {
      setLoading(true);
      try {
        const chapterMeta = chaptersMetadata.find(ch => ch.id === currentChapter);
        if (chapterMeta) {
          const response = await fetch(`/chapters/${chapterMeta.filename}`);
  ...
  const currentChapterMeta = chaptersMetadata.find(ch => ch.id === currentChapter);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      {/* Book Header */}
      <div className="relative bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-white py-12 px-4 mt-16">
          ...
          >
            ...
            >
              <div className="flex items-center justify-center gap-3 mb-3">
                ...
                <h1 className="text-3xl md:text-4xl font-bold">Mastering Fundamental Analysis</h1>
              </div>
              <p className="text-lg text-white/90 mb-2">
                A Complete Guide to In-Depth Stock Evaluation
              </p>
              <p className="text-sm text-white/80">
                Frameworks, Checklists, and Practical Methods for Business Quality and Fair Value
              </p>
            </a>
            <div className="flex items-center justify-center gap-6 mt-4 text-sm">
              <span className="flex items-center gap-2">
                <BookOpen className="w-4 h-4" />
                {chaptersMetadata.length} Chapters
              </span>
              <span className="flex items-center gap-2">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          <a
            href="/Mastering_Fundamental_Analysis.md"
            download
          ...
          <aside className="hidden lg:block w-80 flex-shrink-0">
            <div className="sticky top-20 bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
              <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-4">
                ...
                <p className="text-xs text-white/80 mt-1">{chaptersMetadata.length} Chapters</p>
              </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="lg:hidden mb-4">
              ...
              >
                ...
                <span>{showTOC ? 'Hide' : 'Show'} All Chapters ({chaptersMetadata.length})</span>
              </button>
            </div>
              ...
              >
                ...
                <div className="flex items-center gap-4 mb-6 pb-4 border-b border-gray-200 dark:border-gray-700">
                  <span className="text-sm text-gray-600 dark:text-gray-400">
                    {currentChapter} of {chaptersMetadata.length}
                  </span>
                </div>
                {loading ? (
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-gray-600 dark:text-gray-400 font-medium hidden sm:block">
                {currentChapter} / {chaptersMetadata.length}
              </span>

              <button
                onClick={() => {
                  setCurrentChapter(Math.min(chaptersMetadata.length, currentChapter + 1));
                  window.scrollTo({ top: 0, behavior: 'smooth' });
                }}
                disabled={currentChapter === chaptersMetadata.length}
                className={`flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition-all ${
                  currentChapter === chaptersMetadata.length
                    ? 'bg-gray-200 dark:bg-gray-700 text-gray-400 cursor-not-allowed'
                    : 'bg-blue-600 hover:bg-blue-700 text-white shadow-md hover:shadow-lg'
