            ...
            <ul className="space-y-1" role="menu">
              {subSections.map((section) => (
                <li key={section.id} role="none">
                  <button
                    ...
                    className={`w-full flex items-center space-x-2 px-3 py-2 lg:py-3 rounded-lg font-medium transition-all duration-300 text-left ${
                      activeSubSection === section.id
                        ? 'bg-gradient-to-r from-orange-600 to-red-700 text-white shadow-md'
                  ...
                  >
                    <div className={`${activeSubSection === section.id ? 'text-orange-200' : 'text-gray-500'}`} aria-hidden="true">
                      {section.icon}
                    </div>
                    <div className="flex-1">
                      <div className="font-semibold text-xs lg:text-sm">{section.label}</div>
                      <div className={`text-xs hidden lg:block ${activeSubSection === section.id ? 'text-orange-200' : 'text-gray-500'}`}>{section.description}</div>
                    </div>
                  </button>
                </li>
              ))}
